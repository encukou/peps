...
If an API that prevents adding features or optimizatins, or presents a serious
...
But, it's preferable to avoid such “undesirable” API in *new* code,
especially if a safer alternative exists.
...
Adding the ``Py`` prefix                     --  XXX
------------------------

Some names defined in CPython headers is not namespaced: it that lacks the
``Py`` prefix or variant (``_Py`` and alternative capitalizations).
For example, ``setter`` or  -- hiding the name entirely means that it cannot clash with user
or library code.
...
- Soft-deprecate and omit macros without the ``Py``/``_Py`` prefix
  ...
  - Replacements for macros generated by ``configure``
    ...
    a common prefix) may not be ready for 3.14; if so they'll be omitted with
    later versions of ``Py_COMPAT_API_VERSION``.

...
On systems that use ABI version tagged .so files as introduced in :pep:`3149`
(Linux, macOS, and similar), the extension should be named
...
with *only* ``abi3t``, by defining only :samp:`Py_TARGET_ABI3T={v}`, building
with GIL-enabled CPython, and tagging the resulting wheel with ``abi3t`` rather
than ``abi3.abi3t``.
...
(While this has been the case since Python 3.13, the "GIL-enabled" limitation
does break the of the promise in :pep:`384`.
Or, depending on how you look at it, *will* break the promise if and when
free-threaded builds become default.)
...
In a "softer" but important line of reasoning, the ``abi3.abi3t`` tag is
explicit, answering whether a wheel is still compatible with ``abi3``.

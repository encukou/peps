...
To help you apply the guidelines, they sometimes repeat information.
If you find that they contradict themselves, please ask the C API working group
to clarify.
...
To request an approval, contact any
:ref:`member of the working group <pep731-members>`.
...
use feature flags that users define before including the header
(e.g. ``Py_LIMITED_API``).
...
- C11, with optional features needed by CPython:
  ...
  - IEEE 754 floating point (``__STDC_IEC_559__``)
...
.. TODO: say this is for all C code. Point PEP 7 here: this is not a style issue.
   (it's not just an API issue either, but here's better than PEP 7)
...
you can add additional C/C++-specific API. Usually, the C/C++-specific API
will be either easier to use or more performant.
For example:
...
*  A function that uses C-specific types, such as ``PyLong_AsLongLong``,
   is OK as long as equivalent functions are provided
...
*  As implementation detail of shadowing a function.
...
*  ``size_t``, ``intptr_t``, ``ptrdiff_t`` which necessarily depend on
   the platform
*  ``char``
...
*  Opaque structs, whose members and size are not part of the public API.
   These are handled via function calls.
...
The return value of a function must indicate whether an exception was raised.
...
*  An integral value, where ``-1`` is returned if and only if an exception was
   raised, and other values signal an absence of exception.
...
Note that ``-1`` and ``NULL`` are still reserved for errors;
infallible functions must never return them.
...
that should not be exposed in the API.
...
* An output argument must always initialized (even on errors)
* Output arguments can be ``NULL``; in which case the result is ignored
  (and a ``PyObject*`` result is not incref'd).
...
object, and should not be used if the object is no longer “fresh”.
...
A *reference* is a pointer with extra abstract semantics, which comes in
two flavors:
...
*  A **borrowed** reference means a copy of a reference held by another
   owner, with some guarantee in place that the it will stay valid
...
* treat it as *borrowed* from the new owner, if the new owner guarantees
  the reference stays valid.

There are two kinds of resources with managed ownership:

*  ``PyObject *``, which we used for all reference-counted resources.
...
*  Only add a function that return borrowed references as an alternative
...
When designing documenting lifetime and ownership, pay special attention to
...
Treat strings (``const char*``) are non-reference-counted references.
...
*  Lookup (“getattr”, “getitem” or “setdefault” style) functions,
   where the lookup result is passed via an
   :ref:`output argument <output argument>` (``int``):
